...
def get_args():
    ...
    parser.add_argument("--inference_data", type=str, default="path_to/output/llava-1.5_AMBER.jsonl") # TODO 待修改 以及evaluation_type
...
def main(args):
    ...
    for i in tqdm(range(len(inference_data))):
        ...
        if ground_truth[id-1]['type'] == 'generative':
            ...
            nouns = extract_nouns(inference_data[i]['output'])
            after_process_nouns = []  # 没看懂含义
            for noun in nouns:
                if noun in hallucination_words:
                    after_process_nouns.append(noun)
            
            safe_words = [] # 包含从 association 字典中提取的所有关联词
            safe_list = [] # 包含这些关联词在truth中的索引，每个索引重复的次数等于有多少个关联词
            ...
            response = inference_data[i]['output']
            # response = response.split(',')[0] # TODO 不使用recorder
            response = recorder(response) # 使用recorder
            if truth == 'yes':
                if response == 'Yes':
